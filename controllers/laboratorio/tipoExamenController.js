...
module.exports = {
    ...
    list(req, res) {
        ...
        TipoExamen.findAndCountAll({ where: condition,order:[[`${criterio}`,`${order}`]],limit,offset})
        .then(data => {
    ...
    get (req, res) {
        TipoExamen.findAll({attributes: ['id', 'nombre']}, {
            where: { estado: 1 }})
        .then(data => {
            res.send(data);
        })
