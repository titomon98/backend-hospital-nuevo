...
module.exports = {
    create(req, res) {
        ...
        const datos = {
            ...
            categoria: form.categoria.nombre,
            estado: 1
        };
    ...
    list(req, res) {
        ...
        Personal.findAndCountAll({where: condition,order:[[`${criterio}`,`${order}`]],limit,offset})
        .then(data => {
    ...
    find (req, res) {
        ...
        .then(medico => res.status(200).send(medico))
    ...
    update (req, res) {
        ...
        .then(medico => res.status(200).send('El registro ha sido actualizado'))
        .catch(error => {
            console.log(error)
            return res.status(400).json({ msg: 'Ha ocurrido un error, por favor intente más tarde' });
        });
    ...
    get (req, res) {
        Personal.findAll({attributes: ['id', 'nombre']})
        .then(data => {
            res.send(data);
        })
        .catch(error => {
